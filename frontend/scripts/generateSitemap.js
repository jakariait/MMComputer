...
const FRONTEND_URL = process.env.FRONTEND_URL || 'https://mmcomputerbd.com';
...
const generateSitemap = async () => {
  ...
  try {
    ...
    if (totalPages > 1) {
      ...
      for (const res of restResponses) {
        const pageData = res.data?.products || res.data?.data || res.data || [];
  ...
  const staticPages = [
    ...
    { loc: '/pc-builder', changefreq: 'weekly', priority: 0.8 },
    { loc: '/feedback-complain', changefreq: 'monthly', priority: 0.5 },
    { loc: '/brands', changefreq: 'weekly', priority: 0.6 },
    { loc: '/warranty', changefreq: 'monthly', priority: 0.5 },
    { loc: '/repair-service', changefreq: 'monthly', priority: 0.5 },
